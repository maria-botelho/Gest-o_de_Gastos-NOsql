...
def popular():
    ...
    db.usuarios.drop(); db.contas.drop(); db.transacoes.drop(); db.categorias.drop()
    
    print("📦 Inserindo categorias...")
    inserir_categorias_padrao()
    
    print("👤 Criando Ana Silva...")
    uid = inserir_usuario("Ana Silva", "ana@email.com")
    cid_corrente = inserir_conta(uid, "Nubank Corrente", "corrente")
    cid_poupanca = inserir_conta(uid, "Itaú Poupança", "poupança")
    
    hoje = datetime.now() # Maio de 2026 conforme o seu sistema
